...
const UserTree = ({ treeData }) => {
  // Destructure treeData from props
  ...
  const zoomIn = (factor = 1.1) =>
    setZoomLevel((zoomLevel) => zoomLevel * factor); // Function to zoom in
  const zoomOut = (factor = 1.1) =>
    setZoomLevel((zoomLevel) => zoomLevel / factor); // Function to zoom out
  ...
  const handleWheel = (event) => {
    ...
    const userTreeElement =
      treeContainerRef.current.querySelector(".user-tree");
    if (userTreeElement) {
    ...
    if (nodeElement) {
      nodeElement.onmouseleave = () => {
        ...
        const currentNode = currentId
          ? treeData.nodes.find((n) => n.currentId === currentId)
          : null;
        const parsedContent = parseQuestionContent(
          currentNode ? currentNode.content : rootQuestion
        );
        setCurrentNodeContent(parsedContent);
      };
    }
    ...
    const placeNode = (nodeId, level, maxLevel) => {
      ...
      if (node) {
        ...
        nodeElement = (
          <NodeComponent
            ...
            onMouseEnter={() => handleNodeHover(node.currentId)}
          />
        );
        children = [];
        ...
        nodeElement = (
          <InvisibleNodeComponent key={`invisible-${level}-${Math.random()}`} />
        );
        children = [
      ...
      return (
        <div
          ...
          key={nodeId || `level-${level}-${Math.random()}`}
        >
          {nodeElement}
  ...
  const colorNodes = () => {
    ...
    document.querySelectorAll(".node-component").forEach((element) => {
      ...
      element.style.backgroundColor =
        element.id === currentNodeId ? greenNode : blueNode;
    });
    ...
    treeData.nodes.forEach((node) => {
      ...
      if (element) {
        const nodeColor =
          node.currentId === currentNodeId
            ? greenNode
            : node.yesChildId || node.noChildId
            ? blueNode
            : yellowNode;
  ...
  const handleYes = () => {
    ...
    if (currentNode && currentNode.yesChildId) {
      ...
      const childNode = treeData.nodes.find(
        (n) => n.currentId === currentNode.yesChildId
      );
  ...
  const handleNo = () => {
    ...
    if (currentNode && currentNode.noChildId) {
      ...
      const childNode = treeData.nodes.find(
        (n) => n.currentId === currentNode.noChildId
      );
      setCurrentNodeContent(childNode ? childNode.content : "");
    } else if (!currentNodeId && treeData.nodes.length > 0) {
  ...
  const handleBack = () => {
    ...
    if (currentNode && currentNode.parentId) {
      ...
      const parentNode = treeData.nodes.find(
        (n) => n.currentId === currentNode.parentId
      );
  ...
  return (
    <>
      <div id="user-tree-page">
        <h1 id="tree-name" className={zoomLevel > 1 ? "hide-title" : ""}>
          {treeName}
        </h1>
        ...
        <div className="user-tree-container" ref={treeContainerRef}>
          ...
          >
            {nodeRows.map((nodeRow, index) => (
              <div
                key={`node-row-${index}`}
                className={`node-row level-${index}`}
              >
                {nodeRow}
              </div>
            ))}
          ...
          <CurrentNodeDetails
            ...
            onYes={handleYes}
            showBack={
              currentNodeId && currentNodeId !== treeData.nodes[0].currentId
            } // Show back button if not at root
            showYes={
              !!treeData.nodes.find((n) => n.currentId === currentNodeId)
                ?.yesChildId
            } // Show "Yes" if there is a yesChildId
            showNo={
              !!treeData.nodes.find((n) => n.currentId === currentNodeId)
                ?.noChildId
            } // Show "No" if there is a noChildId
            question={
              <div dangerouslySetInnerHTML={{ __html: currentNodeContent }} />
            } // Display the current node's content, display root question if not set
          />
        </div>
      </div>
    </>
